...
export default function LessonFormModal({ isOpen, lesson, assignmentMode = false, nextOrderIndex = 0, isLoading, serverNameError, serverError, onSubmit, onClose }: LessonFormModalProps) {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<LessonFormValues>({
    ...
    defaultValues: { name: '', description: '', requiredScore: 70, questionCount: assignmentMode ? 0 : 10, xpReward: 50, diamondReward: 5, orderIndex: nextOrderIndex, status: 'DRAFT' },
  })

  useEffect(() => {
    if (!isOpen) return
    reset(lesson ? { name: lesson.name, description: lesson.description ?? '', requiredScore: lesson.requiredScore, questionCount: lesson.questionCount, xpReward: lesson.xpReward, diamondReward: lesson.diamondReward, orderIndex: lesson.orderIndex, status: lesson.status } : { name: '', description: '', requiredScore: 70, questionCount: assignmentMode ? 0 : 10, xpReward: 50, diamondReward: 5, orderIndex: nextOrderIndex, status: 'DRAFT' })
  ...
  const numberFields = allNumberFields.filter((field) => !(assignmentMode && field.key === 'questionCount'))
