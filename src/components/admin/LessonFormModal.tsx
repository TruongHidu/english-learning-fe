...
interface LessonFormModalProps {
  ...
  /** Assignment-created lessons are always DRAFT and start without questions. */
...
export default function LessonFormModal({ isOpen, lesson, assignmentMode = false, nextOrderIndex = 0, isLoading, serverNameError, serverError, onSubmit, onClose }: LessonFormModalProps) {
  const isEdit = Boolean(lesson)
  const { register, handleSubmit, reset, formState: { errors } } = useForm<LessonFormValues>({
    resolver: zodResolver(lessonFormSchema),
    defaultValues: { name: '', description: '', requiredScore: 70, questionCount: 0, xpReward: 50, diamondReward: 5, orderIndex: nextOrderIndex, status: 'DRAFT' },
  })

  useEffect(() => {
    if (!isOpen) return
    reset(lesson ? { name: lesson.name, description: lesson.description ?? '', requiredScore: lesson.requiredScore, questionCount: lesson.questionCount, xpReward: lesson.xpReward, diamondReward: lesson.diamondReward, orderIndex: lesson.orderIndex, status: lesson.status } : { name: '', description: '', requiredScore: 70, questionCount: 0, xpReward: 50, diamondReward: 5, orderIndex: nextOrderIndex, status: 'DRAFT' })
  }, [assignmentMode, isOpen, lesson, nextOrderIndex, reset])

  if (!isOpen) return null

  const allNumberFields: Array<{ key: 'requiredScore' | 'questionCount' | 'xpReward' | 'diamondReward'; label: string; min: number; max?: number }> = [
    { key: 'requiredScore', label: 'Điểm yêu cầu (%)', min: 0, max: 100 },
    { key: 'questionCount', label: 'Số câu hỏi', min: 0, max: 100 },
    { key: 'xpReward', label: 'XP thưởng', min: 0 },
    { key: 'diamondReward', label: 'Kim cương thưởng', min: 0 },
  ]
  const numberFields = allNumberFields.filter((field) => field.key !== 'questionCount')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="lesson-form-title">
      <div className="w-full max-w-2xl bg-white rounded-2xl p-6 shadow-2xl border-2 border-slate-200 overflow-y-auto max-h-[92vh]">
        ...
        {serverError ? <p className="mb-3 rounded-lg bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-600" role="alert">{serverError}</p> : null}
        <p className="mb-3 text-sm text-slate-500">{lesson ? `${lesson.publishedQuestionCount ?? 0}/${lesson.assignedQuestionCount ?? lesson.questionCount} câu hỏi đã xuất bản/đã gán` : 'Tạo bài nháp, gán câu hỏi đã xuất bản rồi phát hành bài học.'}</p>
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          {!isEdit ? <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block text-xs font-extrabold text-slate-700 mb-1.5" htmlFor="lesson-order">Thứ tự *</label><input id="lesson-order" type="number" min={0} className="admin-field" {...register('orderIndex', { valueAsNumber: true })} />{errors.orderIndex ? <p className="text-rose-600 text-xs mt-1">{errors.orderIndex.message}</p> : null}</div>{!assignmentMode ? <div><label className="block text-xs font-extrabold text-slate-700 mb-1.5" htmlFor="lesson-status">Trạng thái</label><select id="lesson-status" className="admin-field" {...register('status')}><option value="DRAFT">Bản nháp</option><option value="INACTIVE">Ngừng sử dụng</option></select></div> : <p className="self-end text-xs font-semibold text-slate-500">Lesson mới sẽ được tạo ở trạng thái DRAFT và gán câu hỏi sau khi tạo.</p>}</div> : null}
